...
#   ]
# 
#Funcion de procesamiento. Retornar una tupla la linea de archivo y la linea fisica.
import glob
import fileinput

def load_input(input_directory):

    """Carga los archivos y generación de tupla """
    filesnames=glob.glob(input_directory + "/*.*")

    sequence = []   
    with fileinput.input(files=filesnames) as f:
        for line in f:
            sequence.append(
                (f.filename(), line )
            )    
...
# texto.
#
#group by es muy eficiente para procesarlas 
...
#buscar si el directorio existe y crear uno si efectivamente no existe.
...
def create_marker(output_directory):
    with open(os.path.join(output_directory, "SUCCESS"),"w") as f:
        f.write("")
